...
//the next three functions handle the strain rate bits
...
struct interact strainz(Atom* one)
{
	if(one->getType() == EDGEATOM)
		one->translate(Vector3D(0,0,signf(one->getPosition().getY())*strainZrate*Assemblage::getdt()));
...
private:
	...
	GlowPushButtonWidget* _initFCC100PushButton;
	GlowPushButtonWidget* _initFCC110PushButton;
	GlowPushButtonWidget* _initFCC111PushButton;
	...
	GlowLabeledTextFieldWidget* _strainZText;
...
GlowWindow("Stack 3D", GlowWindow::autoPosition, GlowWindow::autoPosition,320, 320, Glow::rgbaBuffer, Glow::noEvents)	//obligatory superclass constructor call
{
	myfps = new fpscounter(13);	//create a new fps counter

	myassemblage = new Assemblage(Vector3D(-4,-8,-4), Vector3D(4,8,4));	//create an assemblage
	myassemblage->showboundaries(true);	//we want to see our boundaries
	myassemblage->setjigglemask(true, true, true);	//and giggle every which way
	myassemblage->cutoff(2.5);
	...
	_controlsWindow = new GlowQuickPaletteWindow("Controls");
		panel1 = _controlsWindow->AddArrangingPanel(GlowQuickPalette::horizontal, GlowQuickPalette::alignCenter);
			_initFCC100PushButton = panel1->AddPushButton("Init FCC (100)", this);
			_initFCC110PushButton = panel1->AddPushButton("Init FCC (110)", this);

		panel1 = _controlsWindow->AddArrangingPanel(GlowQuickPalette::horizontal, GlowQuickPalette::alignCenter);
			_initFCC111PushButton = panel1->AddPushButton("Init FCC (111)", this);
		...
		panel1 = _controlsWindow->AddPanel(GlowQuickPanelWidget::etchedStyle, "System");
			...
			strainZrate = 0.0;
			_strainZText = panel1->AddTextField(100, "0.0", "Strain Rate (z-axis)");
		...
		panel1 = _controlsWindow->AddPanel(GlowQuickPanelWidget::etchedStyle, "Draw Style");
			...
			_emaxText = _energyPanel->AddTextField(100, "-7.0", "E max");
			_eminText = _energyPanel->AddTextField(100, "-15.0", "E min");
...
void AssemblageWindow::OnMessage(const GlowPushButtonMessage& message)	//on receipt of a push button message
{
	...
	//the following constants are used by the various initialization functions
	const double sqrt2 = sqrt(2.0);
	const double sqrt3 = sqrt(3.0);
	const double sqrt2o3 = sqrt2/sqrt3;
	const double athird = 1.0/3.0;

	//all three initialization methods get somewhat hairy and contain many loops and conditionals, I recommend referring to stack2d which is almost identical but simpler
	if(message.widget == _initFCC100PushButton)
	{
		...
		for(i=-6; i<=6; ++i)
		{
			...
			if(abs(i+6)%2 == 0)
			{
				for(j=0; j<6; ++j)
					for(k=0; k<6; ++k)
					{
						temp = new Atom(newtype, Vector3D(-2.5+(double)j, 0.5*sqrt2*(double)i, -2.5+(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
			...
			else
			{
				for(j=0; j<5; ++j)
					for(k=0; k<5; ++k)
					{
						temp = new Atom(newtype, Vector3D(-2.0+(double)j, 0.5*sqrt2*(double)i, -2.0+(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
						myassemblage->addAtom(temp);
					}
			}
		}
	...
	//all three initialization methods get somewhat hairy and contain many loops and conditionals, I recommend referring to stack2d which is almost identical but simpler
	if(message.widget == _initFCC110PushButton)
	{
		if(myassemblage->numAtoms() >0 )
		{
			fprintf(stdout, "Clearing old Atoms\n");
		...
		for(i=-8; i<=8; ++i)
		{
			...
			if(abs(i+8)%2 == 0)
			{
				for(j=0; j<6; ++j)
					for(k=0; k<5; ++k)
					{
						temp = new Atom(newtype, Vector3D(-2.5+(double)j, 0.5*(double)i, -2.0*sqrt2+sqrt2*(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
						myassemblage->addAtom(temp);
					}
			}
			else
			{
				for(j=0; j<5; ++j)
					for(k=0; k<4; ++k)
					{
						temp = new Atom(newtype, Vector3D(-2.0+(double)j, 0.5*(double)i, -1.5*sqrt2+sqrt2*(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
						myassemblage->addAtom(temp);
					}
			}
		}
	...
	//all three initialization methods get somewhat hairy and contain many loops and conditionals, I recommend referring to stack2d which is almost identical but simpler
	if(message.widget == _initFCC111PushButton)
	{
		if(myassemblage->numAtoms() >0 )
		{
			fprintf(stdout, "Clearing old Atoms\n");
			...
			if(abs(i+6)%3 == 0)
			{
				for(j=0; j<=6; j++)
				{
					if(j%2 == 0)
					{
						for(k=0; k<=5; k++)
						{
							temp = new Atom(newtype, Vector3D(sqrt3*(-1.5+0.5*(double)j), sqrt2o3*(double)i, 2.5-1.0*(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
							myassemblage->addAtom(temp);
						}
					}
					else
					{
						for(k=1; k<=5; k++)
						{
							temp = new Atom(newtype, Vector3D(sqrt3*(-1.5+0.5*(double)j), sqrt2o3*(double)i, 3.0-1.0*(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
							myassemblage->addAtom(temp);
						}
					}
				}
			...
			else if(abs(i+6)%3 == 1)
			{
				for(j=0; j<=5; j++)
				{
					if(j%2 == 0)
					{
						for(k=0; k<=5; k++)
						{
							temp = new Atom(newtype, Vector3D(sqrt3*(-1.5+athird+0.5*(double)j), sqrt2o3*(double)i, 2.5-1.0*(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
							myassemblage->addAtom(temp);
						}
					}
					else
					{
						for(k=1; k<=5; k++)
						{
							temp = new Atom(newtype, Vector3D(sqrt3*(-1.5+athird+0.5*(double)j), sqrt2o3*(double)i, 3.0-1.0*(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
							myassemblage->addAtom(temp);
						}
					}
				}
			...
			else
			{
				for(j=-1; j<=5; j++)
				{
					if((j+2)%2 == 0)
					{
						for(k=0; k<=5; k++)
						{
							temp = new Atom(newtype, Vector3D(sqrt3*(-1.5+2.0*athird+0.5*(double)j), sqrt2o3*(double)i, 2.5-1.0*(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
							myassemblage->addAtom(temp);
						}
					}
					else
					{
						for(k=1; k<=5; k++)
						{
							temp = new Atom(newtype, Vector3D(sqrt3*(-1.5+2.0*athird+0.5*(double)j), sqrt2o3*(double)i, 3.0-1.0*(double)k), Vector3D(0.0), 0.5, 1.0, 0.0, defaultColor[0], defaultColor[1], defaultColor[2]);
							myassemblage->addAtom(temp);
						}
					}
				}
	...
	if(message.widget == _setsystemPushButton)	//set the system conditions
	{
		...
		double newstrainZrate = atof(_strainZText->GetTextChars());	//new value for z-strain rate
		...
		sprintf(buffer, "%f", strainZrate);
		_strainZText->SetText(buffer);
...
int main(int argc, char** argv)	//int main(), w00t!
{
	fprintf(stdout, "stack3d - a GAST-Viz application\n");		//print out various bits of header information
