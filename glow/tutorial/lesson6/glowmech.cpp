...
Based on glutmech by Simon Parkinson-Bates.
GLOW widget additions by Daniel Azuma

Basic changes are as follows:

  Changed glutInit() call to Glow::Init().

  Changed glutMainLoop() call to Glow::MainLoop().

  Changed glutIdleFunc() calls to Glow::SetIdleFunc().

  Added a little code to instantiate the controls window.

You can find the exact changes by searching for "FOR GLOW:".
...
#ifdef DRAW_ENVIRO
  glPushMatrix();
  if (mydistance >= 20.136)
    mydistance = 0.0;
  glTranslatef(0.0, -5.0, -mydistance);
...
animation(void)
{
  // FOR GLOW: Current window no longer guaranteed to be set to the drawing
  // window because we've now added a second window.
...
//    glutIdleFunc(animation);
    Glow::SetIdleFunc(animation);  // FOR GLOW: need to replace glutIdleFunc with this
    break;
#endif
  case 2:
//    glutIdleFunc(NULL);
    Glow::SetIdleFunc(NULL);    // FOR GLOW: need to replace glutIdleFunc with this
    break;
  case 3:
    Toggle();
    glutPostRedisplay();
...
//  glutInit(&argc, argv);
  ...
  windowID = glutCreateWindow("glowmech: Vulcan Gunner");
  myinit();
  ...
  // FOR GLOW: Set up controls window
  new MechControls(windowID);
